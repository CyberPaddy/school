...
def send_file_to_socket(sock, FILE, SIZE):
    sock.settimeout(1) # If data could not be sent in 1 second, raise timeout error
    sent_bytes = 0

    # Send 2 byte header which tells how big the file will be
    sock.send(hex_to_two_bytes(str(hex(SIZE))))

    while sent_bytes < SIZE:
        try:
            file_chunk = FILE.read(200)
            sent_bytes += sock.send(file_chunk)
